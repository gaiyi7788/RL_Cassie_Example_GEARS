...
class cassieRLEnvMirror(cassieRLEnvMultiTraj):
	...
	def reset(self, phase=None):
		self.orientation = 0
		self.speed = 0.5
		orientation = self.orientation + random.randint(-30, 30) * np.pi / 100
		...
		self.sim.c = cassie_sim_init("cassie.xml".encode('utf-8'), True)
		self.vis.set_cam("cassie-pelvis", zoom=5, azimuth=90, elevation=-30)
