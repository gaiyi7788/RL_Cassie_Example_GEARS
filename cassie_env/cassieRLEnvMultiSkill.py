...
class cassieRLEnvMultiSkill(cassieRLEnvWithFootForces):
	...
	def compute_reward(self):
		...
		total_reward = 0.7*np.exp(-joint_penalty)+0.3*np.exp(-com_penalty)+0.3*np.exp(-orientation_penalty)

		if self.return_error:
			total_error = np.zeros((20))
			for i in range(10):
				total_error[i] = (ref_pos[joint_index[i]]-self.sim.qpos()[joint_index[i]])
			for i in range(10):
				total_error[10+i] = (ref_vel[vel_index[i]]-self.sim.qvel()[vel_index[i]])
